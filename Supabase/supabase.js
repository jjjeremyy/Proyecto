...
import { fetchConCache, clearCache } from './cache.js'
...
// =============================================
// FUNCIONES DE LECTURA (con caché + paginación)
// =============================================

// ── Artículos publicados (listado paginado) ────────────
// Devuelve solo los campos necesarios para tarjetas.
// page: número de página (0-indexed)
// pageSize: artículos por página
// forceRefresh: si true, ignora el caché
...
// Devuelve los últimos N artículos (por defecto 4).
...
// Devuelve campos mínimos para la búsqueda en cliente.
export async function obtenerArticulosBuscador(forceRefresh = false) {
  ...
  return fetchConCache(cacheKey, async () => {
    ...
    if (error) {
      console.error('Error obteniendo artículos para buscador:', error);
      return null;
    }
    return data;
  }, forceRefresh);
...
// Carga todos los campos necesarios para la vista de detalle.
...
// Devuelve hasta 3 artículos de la misma categoría (excluyendo el actual).
...
// ── Todos los artículos (admin) ────────────────────────
// Sin caché — el admin necesita datos siempre frescos.
...
export async function crearArticulo(nuevoArticulo) {
  ...
  // Invalidar caché de artículos tras creación
  clearCache();
  return data;
}
